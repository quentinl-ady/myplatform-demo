...
export class StandingOrderService {
  ...
  list(userId: string): Observable<StandingOrderListResponse> {
    return this.http.get<StandingOrderListResponse>(`${this.baseUrl}/api/standing-orders/${userId}`);
  }

  get(userId: string, standingOrderId: string): Observable<StandingOrder> {
    return this.http.get<StandingOrder>(`${this.baseUrl}/api/standing-orders/${userId}/${standingOrderId}`);
  }

  update(userId: string, standingOrderId: string, patch: any): Observable<StandingOrder> {
    return this.http.patch<StandingOrder>(`${this.baseUrl}/api/standing-orders/${userId}/${standingOrderId}`, patch);
  }

  delete(userId: string, standingOrderId: string): Observable<void> {
    return this.http.delete<void>(`${this.baseUrl}/api/standing-orders/${userId}/${standingOrderId}`);
  }

  initiate(userId: string, standingOrder: StandingOrderCreateRequest, sdkOutput: string): Observable<StandingOrderInitiateResponse> {
  ...
  finalize(userId: string, standingOrder: StandingOrderCreateRequest, sdkOutput: string): Observable<any> {
